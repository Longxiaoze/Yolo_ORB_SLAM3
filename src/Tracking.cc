...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, const vector<cv::Rect2i> &vPersonArea, string filename)
{
    mImGray = imRGB;
    cv::Mat imDepth = imD;

    /******************* self define ******************/
    mpViewer->mvPersonArea = vPersonArea;
    /******************* self define ******************/
    ...
    mCurrentFrame = Frame(vPersonArea, mImGray,imDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
